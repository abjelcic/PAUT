                                                    ...
                                                    )
{
    ...
    auto CylLineIntersection = [] ( double R , double L , Vec<double,3> In , Vec<double,3> Out ) -> Vec<double,3>
    {
        #ifdef DEBUG
        ...
        double a = v(2)*v(2) + v(3)*v(3);
        double b = 2*( (In(2)-L/2)*v(2) + (In(3)-R)*v(3) );
        double c = (In(2)-L/2)*(In(2)-L/2) + (In(3)-R)*(In(3)-R) - R*R;
    ...
    for( unsigned i = 0 ; i < SourcePoints.size() ; ++i )
    {
        ...
        for( unsigned j = 0 ; j < SourcePoints[i].size(); ++j )
        {
            ...
            auto const F      = Focus;
            auto const Eguess = ( j!=0 ) ? ExitPoints_i[j-1] : CylLineIntersection( wedge.m_R , wedge.m_L , S , F );

            auto const E      = solveCylSnell( wedge.m_c , specimen.m_c , wedge.m_R , wedge.m_L , S , F , Eguess );
